...
const AUDIT_FUNCTION_URL = process.env.AUDIT_FUNCTION_URL || '';

app.use(cors());
app.use(express.json());

// Add X-Served-By header to all responses
...
async function fireAuditWebhook(action, asset) {
  if (!AUDIT_FUNCTION_URL) return;
  ...
  try {
    const resp = await fetch(AUDIT_FUNCTION_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
      signal: AbortSignal.timeout(5000),
    });
    console.log(`Audit webhook (${action}): ${resp.status}`);
  } catch (err) {
    console.error(`Audit webhook failed (${action}): ${err.message}`);
  }
}
