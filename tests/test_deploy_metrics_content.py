"""Content-presence unit tests for VCF 9 Deploy Metrics — VKS Metrics Observability."""
...
class TestShebangAndStrictMode:
    ...
    def test_deploy_first_line_is_bash_shebang(self, metrics_deploy_text):
        first_line = metrics_deploy_text.splitlines()[0]
        assert first_line == "#!/bin/bash", (
            f"Expected '#!/bin/bash' as first line, got '{first_line}'"
        )

    def test_deploy_set_euo_pipefail(self, metrics_deploy_text):
        assert "set -euo pipefail" in metrics_deploy_text, (
            "Deploy script does not contain 'set -euo pipefail'"
        )

    def test_teardown_first_line_is_bash_shebang(self, metrics_teardown_text):
        first_line = metrics_teardown_text.splitlines()[0]
        assert first_line == "#!/bin/bash", (
            f"Expected '#!/bin/bash' as first line, got '{first_line}'"
        )

    def test_teardown_set_euo_pipefail(self, metrics_teardown_text):
        assert "set -euo pipefail" in metrics_teardown_text, (
...
class TestVariableBlockCompleteness:
    ...
    def test_all_required_variables_defined(self, metrics_deploy_text):
        for var in self.REQUIRED_VARIABLES:
            pattern = rf'^{var}='
            assert re.search(pattern, metrics_deploy_text, re.MULTILINE), (
...
class TestVariableDefaults:
    ...
    def test_default_variables_use_default_pattern(self, metrics_deploy_text):
...
class TestValidateVariables:
    ...
    def test_validate_variables_function_exists(self, metrics_deploy_text):
        assert "validate_variables" in metrics_deploy_text, (
            "validate_variables function/block not found in deploy script"
        )

    def test_validate_variables_called_before_provisioning(self, metrics_deploy_text):
        call_match = re.search(r"^validate_variables\s*$", metrics_deploy_text, re.MULTILINE)
        ...
        first_prov_pos = len(metrics_deploy_text)
        for pat in prov_patterns:
            m = re.search(pat, metrics_deploy_text)
...
class TestKubeconfigSetup:
    ...
    def test_export_kubeconfig_present(self, metrics_deploy_text):
        assert re.search(r"export\s+KUBECONFIG=", metrics_deploy_text), (
            "Deploy script missing 'export KUBECONFIG' command"
        )

    def test_kubectl_get_namespaces_connectivity_check(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+get\s+namespaces", metrics_deploy_text), (
            "Deploy script missing 'kubectl get namespaces' connectivity check"
        )

    def test_exit_code_2_for_kubeconfig_failures(self, metrics_deploy_text):
        assert "exit 2" in metrics_deploy_text, (
...
class TestNodeSizingAdvisory:
    ...
    def test_kubectl_get_nodes_present(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+get\s+nodes", metrics_deploy_text), (
            "Deploy script missing 'kubectl get nodes' for resource query"
        )

    def test_best_effort_large_recommendation(self, metrics_deploy_text):
        assert "best-effort-large" in metrics_deploy_text, (
            "Deploy script missing 'best-effort-large' recommendation"
        )

    def test_node_cpu_threshold_referenced(self, metrics_deploy_text):
        assert "NODE_CPU_THRESHOLD" in metrics_deploy_text, (
...
class TestNamespaceCreation:
    ...
    def test_kubectl_create_ns_present(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+create\s+ns", metrics_deploy_text), (
            "Deploy script missing 'kubectl create ns'"
        )

    def test_idempotency_check_before_create(self, metrics_deploy_text):
        get_match = re.search(r"kubectl\s+get\s+ns", metrics_deploy_text)
        create_match = re.search(r"kubectl\s+create\s+ns", metrics_deploy_text)
...
class TestRepositoryRegistration:
    ...
    def test_vcf_package_repository_add_present(self, metrics_deploy_text):
        assert re.search(r"vcf\s+package\s+repository\s+add", metrics_deploy_text), (
            "Deploy script missing 'vcf package repository add'"
        )

    def test_wait_for_condition_repo_reconciliation(self, metrics_deploy_text):
        assert "wait_for_condition" in metrics_deploy_text, (
...
class TestPackageInstallCommands:
    ...
    def test_telegraf_install_with_package_name(self, metrics_deploy_text):
        assert re.search(
            r"vcf\s+package\s+install\s+telegraf", metrics_deploy_text
        ), "Deploy script missing 'vcf package install telegraf'"
        assert "telegraf.kubernetes.vmware.com" in metrics_deploy_text, (
            "Deploy script missing --package-name telegraf.kubernetes.vmware.com"
        )

    def test_certmanager_install_with_package_name(self, metrics_deploy_text):
        assert re.search(
            r"vcf\s+package\s+install\s+cert-manager", metrics_deploy_text
        ), "Deploy script missing 'vcf package install cert-manager'"
        assert "cert-manager.kubernetes.vmware.com" in metrics_deploy_text, (
            "Deploy script missing --package-name cert-manager.kubernetes.vmware.com"
        )

    def test_contour_install_with_package_name(self, metrics_deploy_text):
        assert re.search(
            r"vcf\s+package\s+install\s+contour", metrics_deploy_text
        ), "Deploy script missing 'vcf package install contour'"
        assert "contour.kubernetes.vmware.com" in metrics_deploy_text, (
            "Deploy script missing --package-name contour.kubernetes.vmware.com"
        )

    def test_prometheus_install_with_package_name(self, metrics_deploy_text):
        assert re.search(
            r"vcf\s+package\s+install\s+prometheus", metrics_deploy_text
        ), "Deploy script missing 'vcf package install prometheus'"
        assert "prometheus.kubernetes.vmware.com" in metrics_deploy_text, (
            "Deploy script missing --package-name prometheus.kubernetes.vmware.com"
        )

    def test_telegraf_install_has_values_file(self, metrics_deploy_text):
        assert "--values-file" in metrics_deploy_text, (
            "Deploy script missing --values-file flag on Telegraf install"
        )

    def test_prometheus_install_has_storage_class(self, metrics_deploy_text):
        assert "PROMETHEUS_VALUES_FILE" in metrics_deploy_text, (
...
class TestDependencyOrdering:
    ...
    def test_telegraf_after_repo_registration(self, metrics_deploy_text):
        repo_match = re.search(r"vcf\s+package\s+repository\s+add", metrics_deploy_text)
        telegraf_match = re.search(r"vcf\s+package\s+install\s+telegraf", metrics_deploy_text)
    ...
    def test_certmanager_after_repo_registration(self, metrics_deploy_text):
        repo_match = re.search(r"vcf\s+package\s+repository\s+add", metrics_deploy_text)
        cm_match = re.search(r"vcf\s+package\s+install\s+cert-manager", metrics_deploy_text)
    ...
    def test_contour_after_certmanager(self, metrics_deploy_text):
        cm_match = re.search(r"vcf\s+package\s+install\s+cert-manager", metrics_deploy_text)
        contour_match = re.search(r"vcf\s+package\s+install\s+contour", metrics_deploy_text)
    ...
    def test_prometheus_after_contour(self, metrics_deploy_text):
        contour_match = re.search(r"vcf\s+package\s+install\s+contour", metrics_deploy_text)
        prom_match = re.search(r"vcf\s+package\s+install\s+prometheus", metrics_deploy_text)
...
class TestVerificationPhase:
    ...
    def test_vcf_package_installed_list_present(self, metrics_deploy_text):
        assert re.search(
            r"vcf\s+package\s+installed\s+list", metrics_deploy_text
        ), "Deploy script missing 'vcf package installed list'"

    def test_telegraf_pod_check_present(self, metrics_deploy_text):
        # Check for kubectl get pods with telegraf reference
        assert re.search(
            r"kubectl\s+get\s+pods.*telegraf", metrics_deploy_text
        ), "Deploy script missing Telegraf pod check"

    def test_prometheus_pod_check_present(self, metrics_deploy_text):
        assert re.search(
            r"kubectl\s+get\s+pods.*prometheus", metrics_deploy_text
...
class TestExitCodes:
    ...
    def test_exit_codes_1_through_10_present(self, metrics_deploy_text):
        for code in range(1, 11):
            assert f"exit {code}" in metrics_deploy_text, (
                f"Deploy script missing 'exit {code}'"
            )

    def test_exit_0_present(self, metrics_deploy_text):
        assert "exit 0" in metrics_deploy_text, (
...
class TestSummaryBanner:
    ...
    def test_deploy_metrics_in_deploy(self, metrics_deploy_text):
        assert "Deploy Metrics" in metrics_deploy_text, (
            "Deploy script missing 'Deploy Metrics' in summary banner"
        )

    def test_deployment_complete_in_deploy(self, metrics_deploy_text):
        assert "Deployment Complete" in metrics_deploy_text, (
...
class TestTeardownOrdering:
    ...
    def test_prometheus_before_contour(self, metrics_teardown_text):
        prom_match = re.search(r"delete_package\s+prometheus", metrics_teardown_text)
        contour_match = re.search(r"delete_package\s+contour", metrics_teardown_text)
    ...
    def test_contour_before_certmanager(self, metrics_teardown_text):
        contour_match = re.search(r"delete_package\s+contour", metrics_teardown_text)
        cm_match = re.search(r"delete_package\s+cert-manager", metrics_teardown_text)
    ...
    def test_certmanager_before_telegraf(self, metrics_teardown_text):
        cm_match = re.search(r"delete_package\s+cert-manager", metrics_teardown_text)
        telegraf_match = re.search(r"delete_package\s+telegraf", metrics_teardown_text)
    ...
    def test_telegraf_before_repo_delete(self, metrics_teardown_text):
        telegraf_match = re.search(r"delete_package\s+telegraf", metrics_teardown_text)
        repo_match = re.search(r"kubectl\s+delete\s+packagerepository", metrics_teardown_text)
    ...
    def test_repo_before_namespace_delete(self, metrics_teardown_text):
        repo_match = re.search(r"kubectl\s+delete\s+packagerepository", metrics_teardown_text)
        ns_match = re.search(r'kubectl\s+delete\s+ns\s+"\$\{PACKAGE_NAMESPACE\}"', metrics_teardown_text)
    ...
    def test_grafana_before_prometheus(self, metrics_teardown_text):
        grafana_match = re.search(r"helm\s+uninstall\s+grafana-operator", metrics_teardown_text)
        prom_match = re.search(r"delete_package\s+prometheus", metrics_teardown_text)
...
class TestTeardownNonInteractive:
    ...
    def test_delete_package_helper_strips_finalizers(self, metrics_teardown_text):
        # The delete_package helper strips finalizers before deleting
        assert re.search(
            r'kubectl\s+patch\s+packageinstall.*finalizers', metrics_teardown_text, re.DOTALL
        ), "delete_package helper missing finalizer stripping for PackageInstall"
        assert re.search(
            r'kubectl\s+patch\s+app.*finalizers', metrics_teardown_text, re.DOTALL
        ), "delete_package helper missing finalizer stripping for App"

    def test_all_four_packages_use_delete_package(self, metrics_teardown_text):
        # Verify all 4 packages are deleted via the delete_package helper
        for pkg in ["prometheus", "contour", "cert-manager", "telegraf"]:
            assert re.search(rf"delete_package\s+{re.escape(pkg)}", metrics_teardown_text), (
                f"Package '{pkg}' not deleted via delete_package helper"
            )

    def test_delete_package_helper_uses_ignore_not_found(self, metrics_teardown_text):
        # The delete_package helper uses --ignore-not-found on kubectl delete
        assert re.search(
            r'kubectl\s+delete\s+packageinstall.*--ignore-not-found', metrics_teardown_text
        ), "delete_package helper missing --ignore-not-found on kubectl delete"

    def test_repo_delete_strips_finalizers(self, metrics_teardown_text):
        assert re.search(
            r'kubectl\s+patch\s+packagerepository.*finalizers', metrics_teardown_text, re.DOTALL
...
class TestNodeSizingAdvisoryLogWarn:
    ...
    def test_log_warn_with_sizing_recommendation(self, metrics_deploy_text):
        # Find log_warn calls that mention sizing / best-effort-large
        warn_calls = re.findall(r"log_warn\s+\".*?\"", metrics_deploy_text)
        has_sizing_warn = any(
...
class TestGrafanaDeployContent:
    ...
    def test_helm_install_grafana_operator(self, metrics_deploy_text):
        assert re.search(r"helm\s+.*grafana-operator", metrics_deploy_text), (
            "Deploy script missing Helm install for Grafana Operator"
        )

    def test_kubectl_apply_grafana_instance(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+apply\s+-f.*GRAFANA_INSTANCE", metrics_deploy_text), (
            "Deploy script missing kubectl apply for Grafana instance"
        )

    def test_kubectl_apply_grafana_datasource(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+apply\s+-f.*GRAFANA_DATASOURCE_FILE", metrics_deploy_text), (
            "Deploy script missing kubectl apply for Grafana datasource"
        )

    def test_kubectl_apply_grafana_dashboards(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+apply\s+-f.*GRAFANA_DASHBOARDS_FILE", metrics_deploy_text), (
            "Deploy script missing kubectl apply for Grafana dashboards"
        )

    def test_grafana_pod_check_in_verification(self, metrics_deploy_text):
        assert re.search(r"kubectl\s+get\s+pods.*GRAFANA_NAMESPACE", metrics_deploy_text), (
            "Deploy script missing Grafana pod check in verification phase"
        )

    def test_grafana_after_prometheus(self, metrics_deploy_text):
        prom_match = re.search(r"vcf\s+package\s+install\s+prometheus", metrics_deploy_text)
        grafana_match = re.search(r"helm\s+.*grafana-operator", metrics_deploy_text)
